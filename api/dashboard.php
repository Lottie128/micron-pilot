...
foreach ($parts as &$part) {
    ...
    $inv_stmt = $conn->prepare("
        SELECT s.stage_name, s.stage_order, s.stage_type,
    ...
    foreach ($part['stage_inventory'] as &$stage) {
        $bin_stmt = $conn->prepare("
            ...
            WHERE i.part_id = ? AND s.stage_name = ? AND i.quantity > 0
        ");
        $bin_stmt->execute([$part['id'], $stage['stage_name']]);
...
$stats = [
    ...
    'total_inventory' => array_sum(array_column($parts, 'total_inventory')),
    'total_good' => array_sum(array_column($parts, 'total_good')),
    'total_rework' => array_sum(array_column($parts, 'total_rework')),
    'total_rejected' => array_sum(array_column($parts, 'total_rejected'))
];
